	...
	componentDidMount() {
		...
		if(data === undefined || Number(data) === 1){
			this.setState({index: 1}, () => {
				axios.get("https://distritoeducativo.herokuapp.com/news/:id", {params: {theIndex: this.state.index}, headers: {"Access-Control-Allow-Origin": "*"}, withCredentials: true}).then(response => {
		 			this.setState({news: [...this.state.news, response.data]});
		 			this.setState({news: this.state.news[0]});
		 			axios.get("https://distritoeducativo.herokuapp.com/totalPages", {params: {theCount: this.state.index}, headers: {"Access-Control-Allow-Origin": "*"}, withCredentials: true}).then(responses => {
