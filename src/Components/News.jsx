	...
	componentDidMount() {
		...
		if(data === undefined || Number(data) === 1){
			this.setState({index: 1}, () => {
				axios.get("https://distritoeducativo.herokuapp.com/http/news/:id", {params: {theIndex: this.state.index}}).then(response => {
		 			this.setState({news: [...this.state.news, response.data]});
		 			this.setState({news: this.state.news[0]});
		 			axios.get("https://distritoeducativo.herokuapp.com/http/totalPages", {params: {theCount: this.state.index}}).then(responses => {
